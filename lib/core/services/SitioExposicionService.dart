...
class SitioExposicionService {
  ...
  Future<Database> _initDatabase() async {
    ...
    return await openDatabase(
      ...
      onCreate: (db, version) async {
        ...
        ''');
        await db.execute(
            'CREATE INDEX idx_sitio_exposicion_id ON sitio_exposicion(id_sitio_exposicion)');
      },
    );
  }
  ...
  Future<void> _insertMultipleSitiosExposicion(
      List<Map<String, dynamic>> sitiosList) async {
    try {
      final db = await database;
  ...
  Future<List<Map<String, dynamic>>> listarSitiosExposicion() async {
    ...
    List<Map<String, dynamic>> sitiosLocal =
        await _getAllSitiosExposicionLocal();
  ...
  Future<Map<String, dynamic>> agregarSitioExposicion(
      Map<String, dynamic> nuevoSitio) async {
    try {
        ...
        await _insertMultipleSitiosExposicion([
          {
            ...
            'activo':
                nuevoSitioData['activo'] ? 1 : 0, // SQLite no soporta booleanos
          }
        ]);
  ...
  Future<Map<String, dynamic>> actualizarSitioExposicion(
      int idSitio, Map<String, dynamic> sitioActualizado) async {
    try {
      String url =
          '$BASE_URL/v1/catalogo/captacion/update-sitio-exposicion/$idSitio';
      final response = await httpService.put(url, sitioActualizado);

      if (response.statusCode == 200) {
        final decodedResponse = utf8.decode(response.bodyBytes);
        final sitioActualizadoData =
            jsonDecode(decodedResponse) as Map<String, dynamic>;
        ...
        await _insertMultipleSitiosExposicion([
          {
            ...
            'usuario_modificacion':
                sitioActualizadoData['usuario_modificacion'],
  ...
  Future<void> eliminarSitioExposicion(int idSitio) async {
    try {
      String url =
          '$BASE_URL/v1/catalogo/captacion/delete-sitio-exposicion/$idSitio';
