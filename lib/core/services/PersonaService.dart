...
class PersonaService {
  ...
  Future<Database> _initDatabase() async {
    ...
    return await openDatabase(
      ...
      onCreate: (db, version) async {
        ...
        ''');
        await db
            .execute('CREATE INDEX idx_personas_id ON personas(id_persona)');
        await db
            .execute('CREATE INDEX idx_personas_cedula ON personas(cedula)');
        await db.execute(
            'CREATE INDEX idx_personas_codigo_expediente ON personas(codigo_expediente)');
      },
    );
  }
  ...
  Future<List<Map<String, dynamic>>> _getAllPersonasLocal() async {
    try {
      ...
      final List<Map<String, dynamic>> maps =
          await db.query('personas', orderBy: 'primer_nombre ASC');
      return maps;
    } catch (e) {
  ...
  Future<void> _insertMultiplePersonas(
      List<Map<String, dynamic>> personasList) async {
    try {
      final db = await database;
  ...
  Future<List<Map<String, dynamic>>> buscarPersonasPorNombreOApellido(
    ...
    return personas.where((persona) {
      final primerNombre =
          persona['primer_nombre']?.toString().toLowerCase() ?? '';
      final segundoNombre =
          persona['segundo_nombre']?.toString().toLowerCase() ?? '';
      final primerApellido =
          persona['primer_apellido']?.toString().toLowerCase() ?? '';
      final segundoApellido =
          persona['segundo_apellido']?.toString().toLowerCase() ?? '';
  ...
  Future<List<Map<String, dynamic>>> buscarPersonasPorCedulaOExpediente(
      String busqueda) async {
    List<Map<String, dynamic>> personas = await _getAllPersonasLocal();
    final busquedaLower = busqueda.toLowerCase();
    ...
    return personas.where((persona) {
      ...
      final codigoExpediente =
          persona['codigo_expediente']?.toString().toLowerCase() ?? '';

      return cedula.contains(busquedaLower) ||
          codigoExpediente.contains(busquedaLower);
    }).toList();
  }
