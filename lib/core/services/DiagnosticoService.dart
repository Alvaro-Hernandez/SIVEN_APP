...
class DiagnosticoService {
  ...
  Future<Database> _initDatabase() async {
    ...
    return await openDatabase(
      ...
      onCreate: (db, version) async {
        ...
        ''');
        await db.execute(
            'CREATE INDEX idx_diagnostico_id ON diagnostico(id_diagnostico)');
      },
    );
  }
  ...
  Future<void> _insertMultipleDiagnostico(
      List<Map<String, dynamic>> diagnosticosList) async {
    try {
      final db = await database;
  ...
  Future<List<Map<String, dynamic>>> listarDiagnosticos() async {
    ...
    List<Map<String, dynamic>> diagnosticosLocal =
        await _getAllDiagnosticoLocal();
  ...
  Future<Map<String, dynamic>> agregarDiagnostico(
      Map<String, dynamic> nuevoDiagnostico) async {
    try {
        ...
        await _insertMultipleDiagnostico([
          {
            ...
            'usuario_modificacion':
                nuevoDiagnosticoData['usuario_modificacion'],
            'fecha_modificacion': nuevoDiagnosticoData['fecha_modificacion'],
            'activo': nuevoDiagnosticoData['activo']
                ? 1
  ...
  Future<Map<String, dynamic>> actualizarDiagnostico(
      int idDiagnostico, Map<String, dynamic> diagnosticoActualizado) async {
    try {
      String url =
          '$BASE_URL/v1/catalogo/captacion/update-diagnostico/$idDiagnostico';
      final response = await httpService.put(url, diagnosticoActualizado);

      if (response.statusCode == 200) {
        final decodedResponse = utf8.decode(response.bodyBytes);
        final diagnosticoActualizadoData =
            jsonDecode(decodedResponse) as Map<String, dynamic>;
        ...
        await _insertMultipleDiagnostico([
          {
            ...
            'usuario_modificacion':
                diagnosticoActualizadoData['usuario_modificacion'],
            'fecha_modificacion':
                diagnosticoActualizadoData['fecha_modificacion'],
  ...
  Future<void> eliminarDiagnostico(int idDiagnostico) async {
    try {
      String url =
          '$BASE_URL/v1/catalogo/captacion/delete-diagnostico/$idDiagnostico';
  ...
  Future<void> _fetchAndUpdateDiagnosticos() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        List<Map<String, dynamic>> diagnosticosList =
            jsonResponse.map((diagnostico) {
          return {
