    ...
    try {
      ...
      if (connectivityResult == ConnectivityResult.mobile ||
        ...
        if (response.statusCode == 200) {
          ...
          List<Map<String, dynamic>> captaciones =
              jsonResponse.map((captacion) {
            return {
              ...
              'activo': captacion['activo'] == true || captacion['activo'] == 1
                  ? 1
                  : 0,
          ...
          throw Exception(
              'Error al obtener captaciones: ${response.statusCode}');
        }
      } else {
  ...
  Future<Map<String, dynamic>> _analizarCaptacionesLocalmente({
    ...
    for (var captacion in captaciones) {
      ...
      if (distribucionLocalidad.containsKey(localidad)) {
        distribucionLocalidad[localidad] =
    ...
    for (var captacion in captaciones) {
      String fecha =
          captacion['fechaCaptacion']?.substring(0, 10) ?? 'Desconocido';
    ...
    try {
      ...
      if (connectivityResult == ConnectivityResult.mobile ||
        ...
        if (response.statusCode == 200) {
          ...
          throw Exception(
              'Error al filtrar captaciones: ${response.statusCode}');
        }
      } else {
