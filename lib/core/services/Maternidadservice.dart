...
class MaternidadService {
  ...
  Future<Database> _initDatabase() async {
    ...
    return await openDatabase(
      ...
      onCreate: (db, version) async {
        ...
        ''');
        await db.execute(
            'CREATE INDEX idx_maternidad_id ON maternidad(id_maternidad)');
      },
    );
  }
  ...
  Future<List<Map<String, dynamic>>> _getAllMaternidadLocal() async {
    try {
      ...
      final List<Map<String, dynamic>> maps =
          await db.query('maternidad', orderBy: 'nombre ASC');
      return maps;
    } catch (e) {
  ...
  Future<void> _insertMultipleMaternidad(
      List<Map<String, dynamic>> maternidadList) async {
    try {
      final db = await database;
  ...
  Future<Map<String, dynamic>> agregarMaternidad(
      Map<String, dynamic> nuevaMaternidad) async {
    try {
        ...
        await _insertMultipleMaternidad([
          {
            ...
            'activo': nuevaMaternidadData['activo']
                ? 1
  ...
  Future<Map<String, dynamic>> actualizarMaternidad(
      int idMaternidad, Map<String, dynamic> maternidadActualizada) async {
    try {
      String url =
          '$BASE_URL/v1/catalogo/captacion/update-maternidad/$idMaternidad';
      final response = await httpService.put(url, maternidadActualizada);

      if (response.statusCode == 200) {
        final decodedResponse = utf8.decode(response.bodyBytes);
        final maternidadActualizadaData =
            jsonDecode(decodedResponse) as Map<String, dynamic>;
        ...
        await _insertMultipleMaternidad([
          {
            ...
            'usuario_modificacion':
                maternidadActualizadaData['usuario_modificacion'],
            'fecha_modificacion':
                maternidadActualizadaData['fecha_modificacion'],
  ...
  Future<void> eliminarMaternidad(int idMaternidad) async {
    try {
      String url =
          '$BASE_URL/v1/catalogo/captacion/delete-maternidad/$idMaternidad';
  ...
  Future<void> _fetchAndUpdateMaternidad() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        List<Map<String, dynamic>> maternidadList =
            jsonResponse.map((maternidad) {
          return {
