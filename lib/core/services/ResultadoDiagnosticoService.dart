...
class ResultadoDiagnosticoService {
  ...
  Future<Database> _initDatabase() async {
    ...
    return await openDatabase(
      ...
      onCreate: (db, version) async {
        ...
        ''');
        await db.execute(
            'CREATE INDEX idx_resultado_diagnostico_id ON resultado_diagnostico(id_resultado_diagnostico)');
      },
    );
  }
  ...
  Future<void> _insertMultipleResultadosDiagnostico(
      List<Map<String, dynamic>> resultadosList) async {
    try {
      final db = await database;
  ...
  Future<Map<String, dynamic>?> _getResultadoDiagnosticoByIdLocal(
      int id) async {
    try {
      final db = await database;
      final List<Map<String, dynamic>> maps = await db.query(
  ...
  Future<List<Map<String, dynamic>>> listarResultadosDiagnostico() async {
    ...
    List<Map<String, dynamic>> resultadosLocal =
        await _getAllResultadosDiagnosticoLocal();
  ...
  Future<Map<String, dynamic>> obtenerResultadoDiagnosticoPorId(int id) async {
    ...
    Map<String, dynamic>? resultadoLocal =
        await _getResultadoDiagnosticoByIdLocal(id);
  ...
  Future<Map<String, dynamic>> agregarResultadoDiagnostico(
      Map<String, dynamic> nuevoResultado) async {
    try {
      String url =
          '$BASE_URL/v1/catalogo/captacion/create-resultados-diagnostico';
        ...
        await _insertMultipleResultadosDiagnostico([
          {
            'id_resultado_diagnostico':
                nuevoResultadoData['id_resultado_diagnostico'],
            ...
            'activo': nuevoResultadoData['activo']
                ? 1
  ...
  Future<Map<String, dynamic>> actualizarResultadoDiagnostico(
      int idResultado, Map<String, dynamic> resultadoActualizado) async {
    try {
      String url =
          '$BASE_URL/v1/catalogo/captacion/update-resultados-diagnostico/$idResultado';
      final response = await httpService.put(url, resultadoActualizado);

      if (response.statusCode == 200) {
        final decodedResponse = utf8.decode(response.bodyBytes);
        final resultadoActualizadoData =
            jsonDecode(decodedResponse) as Map<String, dynamic>;
        ...
        await _insertMultipleResultadosDiagnostico([
          {
            'id_resultado_diagnostico':
                resultadoActualizadoData['id_resultado_diagnostico'],
            ...
            'usuario_modificacion':
                resultadoActualizadoData['usuario_modificacion'],
            'fecha_modificacion':
                resultadoActualizadoData['fecha_modificacion'],
  ...
  Future<void> eliminarResultadoDiagnostico(int idResultado) async {
    try {
      String url =
          '$BASE_URL/v1/catalogo/captacion/delete-resultados-diagnostico/$idResultado';
  ...
  Future<void> _fetchAndUpdateResultadosDiagnostico() async {
    try {
      String url =
          '$BASE_URL/v1/catalogo/captacion/list-resultados-diagnostico';
      ...
      if (response.statusCode == 200) {
        ...
        List<Map<String, dynamic>> resultadosList =
            jsonResponse.map((resultado) {
          return {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        await _insertMultipleResultadosDiagnostico([
          {
            'id_resultado_diagnostico':
                resultadoData['id_resultado_diagnostico'],
