...
class LugarIngresoPaisService {
  ...
  Future<Database> _initDatabase() async {
    ...
    return await openDatabase(
      ...
      onCreate: (db, version) async {
        ...
        ''');
        await db.execute(
            'CREATE INDEX idx_lugar_ingreso_pais_id ON lugar_ingreso_pais(id_lugar_ingreso_pais)');
      },
    );
  }
  ...
  Future<void> _insertMultipleLugarIngresoPais(
      List<Map<String, dynamic>> lugaresList) async {
    try {
      final db = await database;
  ...
  Future<Map<String, dynamic>?> _getLugarIngresoPaisByIdLocal(int id) async {
    try {
      ...
      print(
          "Error obteniendo lugar de ingreso por país por ID desde local: $e");
      return null;
    }
  }
  ...
  Future<List<Map<String, dynamic>>> listarLugarIngresoPais() async {
    ...
    List<Map<String, dynamic>> lugaresLocal =
        await _getAllLugarIngresoPaisLocal();
  ...
  Future<Map<String, dynamic>> agregarLugarIngresoPais(
      Map<String, dynamic> nuevoLugar) async {
    try {
        ...
        await _insertMultipleLugarIngresoPais([
          {
            ...
            'activo':
                nuevoLugarData['activo'] ? 1 : 0, // SQLite no soporta booleanos
          }
        ]);
  ...
  Future<Map<String, dynamic>> actualizarLugarIngresoPais(
      int idLugar, Map<String, dynamic> lugarActualizado) async {
    try {
      String url =
          '$BASE_URL/v1/catalogo/captacion/update-lugar-ingreso-pais/$idLugar';
      final response = await httpService.put(url, lugarActualizado);

      if (response.statusCode == 200) {
        final decodedResponse = utf8.decode(response.bodyBytes);
        final lugarActualizadoData =
            jsonDecode(decodedResponse) as Map<String, dynamic>;
        ...
        await _insertMultipleLugarIngresoPais([
          {
            'id_lugar_ingreso_pais':
                lugarActualizadoData['id_lugar_ingreso_pais'],
            ...
            'usuario_modificacion':
                lugarActualizadoData['usuario_modificacion'],
  ...
  Future<void> eliminarLugarIngresoPais(int idLugar) async {
    try {
      String url =
          '$BASE_URL/v1/catalogo/captacion/delete-lugar-ingreso-pais/$idLugar';
