...
class PuestoNotificacionService {
  ...
  Future<Database> _initDatabase() async {
    ...
    return await openDatabase(
      ...
      onCreate: (db, version) async {
        ...
        ''');
        await db.execute(
            'CREATE INDEX idx_puesto_notificacion_id ON puesto_notificacion(id_puesto_notificacion)');
      },
    );
  }
  ...
  Future<void> _insertMultiplePuestosNotificacion(
      List<Map<String, dynamic>> puestosList) async {
    try {
      final db = await database;
  ...
  Future<List<Map<String, dynamic>>> listarPuestosNotificacion() async {
    ...
    List<Map<String, dynamic>> puestosLocal =
        await _getAllPuestosNotificacionLocal();
  ...
  Future<Map<String, dynamic>> obtenerPuestoNotificacionPorId(int id) async {
    ...
    Map<String, dynamic>? puestoLocal =
        await _getPuestoNotificacionByIdLocal(id);
    if (puestoLocal != null) {
  ...
  Future<Map<String, dynamic>> agregarPuestoNotificacion(
      Map<String, dynamic> nuevoPuesto) async {
    try {
  ...
  Future<Map<String, dynamic>> actualizarPuestoNotificacion(
      int idPuesto, Map<String, dynamic> puestoActualizado) async {
    try {
      String url =
          '$BASE_URL/v1/catalogo/captacion/update-puesto-notificacion/$idPuesto';
      final response = await httpService.put(url, puestoActualizado);

      if (response.statusCode == 200) {
        final decodedResponse = utf8.decode(response.bodyBytes);
        final puestoActualizadoData =
            jsonDecode(decodedResponse) as Map<String, dynamic>;
        ...
        await _insertMultiplePuestosNotificacion([
          {
            'id_puesto_notificacion':
                puestoActualizadoData['id_puesto_notificacion'],
            ...
            'usuario_modificacion':
                puestoActualizadoData['usuario_modificacion'],
  ...
  Future<void> eliminarPuestoNotificacion(int idPuesto) async {
    try {
      String url =
          '$BASE_URL/v1/catalogo/captacion/delete-puesto-notificacion/$idPuesto';
