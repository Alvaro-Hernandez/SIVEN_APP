...
class CatalogServiceRedServicio {
  ...
  Future<List<Map<String, dynamic>>> getAllSilais() async {
    final response = await httpService.get('$BASE_URL$CATALOGOS_RED_DE_SERVICIO/list-silais');
  ...
  Future<Map<String, dynamic>> createSilais(Map<String, dynamic> silais) async {
    final response = await httpService.post('$BASE_URL$CATALOGOS_RED_DE_SERVICIO/create-silais', silais);
    return jsonDecode(response.body) as Map<String, dynamic>;
  }

  // Actualizar un SILAIS
  Future<Map<String, dynamic>> updateSilais(int id, Map<String, dynamic> silais) async {
    final response = await httpService.put('$BASE_URL$CATALOGOS_RED_DE_SERVICIO/update-silais/$id', silais);
    return jsonDecode(response.body) as Map<String, dynamic>;
  }
  ...
  Future<List<dynamic>> getAllEstablecimientos() async {
  ...
  Future<List<Map<String, dynamic>>> getEstablecimientosBySilais(int idSilais) async {
    final response = await httpService.get('$BASE_URL$CATALOGOS_RED_DE_SERVICIO/silais/$idSilais/establecimientos');
    List<dynamic> jsonResponse = jsonDecode(response.body);
