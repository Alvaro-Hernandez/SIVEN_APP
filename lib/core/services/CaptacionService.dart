...
class CaptacionService {
  ...
  Future<Map<String, dynamic>> crearCaptacion(
      Map<String, dynamic> nuevaCaptacion) async {
  ...
  Future<void> _fetchAndUpdateCaptaciones() async {
    final response =
        await httpService.get('$BASE_URL/v1/catalogo/captacion/list-captacion');
    if (response.statusCode == 200) {
      ...
      throw Exception(
          'Error al sincronizar captaciones: ${response.statusCode}');
    }
  }
  ...
  Future<void> _insertMultipleCaptaciones(
    ...
    for (var captacion in captaciones) {
      batch.insert('captacion', captacion,
