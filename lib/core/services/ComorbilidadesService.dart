...
class ComorbilidadesService {
  ...
  Future<Database> _initDatabase() async {
    ...
    return await openDatabase(
      ...
      onCreate: (db, version) async {
        ...
        ''');
        await db.execute(
            'CREATE INDEX idx_comorbilidades_id ON comorbilidades(id_comorbilidades)');
      },
    );
  }
  ...
  Future<List<Map<String, dynamic>>> _getAllComorbilidadesLocal() async {
    try {
      ...
      final List<Map<String, dynamic>> maps =
          await db.query('comorbilidades', orderBy: 'nombre ASC');
      return maps;
    } catch (e) {
  ...
  Future<void> _insertMultipleComorbilidades(
      List<Map<String, dynamic>> comorbilidadesList) async {
    try {
      final db = await database;
  ...
  Future<List<Map<String, dynamic>>> listarComorbilidades() async {
    ...
    List<Map<String, dynamic>> comorbilidadesLocal =
        await _getAllComorbilidadesLocal();
  ...
  Future<Map<String, dynamic>> obtenerComorbilidadPorId(int id) async {
    ...
    Map<String, dynamic>? comorbilidadLocal =
        await _getComorbilidadByIdLocal(id);
  ...
  Future<Map<String, dynamic>> agregarComorbilidades(
      Map<String, dynamic> nuevaComorbilidad) async {
    try {
        ...
        await _insertMultipleComorbilidades([
          {
            ...
            'usuario_modificacion':
                nuevaComorbilidadData['usuario_modificacion'],
            'fecha_modificacion': nuevaComorbilidadData['fecha_modificacion'],
            'activo': nuevaComorbilidadData['activo']
                ? 1
  ...
  Future<Map<String, dynamic>> actualizarComorbilidades(
      int idComorbilidad, Map<String, dynamic> comorbilidadActualizada) async {
    try {
      String url =
          '$BASE_URL/v1/catalogo/captacion/update-comorbilidades/$idComorbilidad';
      final response = await httpService.put(url, comorbilidadActualizada);

      if (response.statusCode == 200) {
        final decodedResponse = utf8.decode(response.bodyBytes);
        final comorbilidadActualizadaData =
            jsonDecode(decodedResponse) as Map<String, dynamic>;
        ...
        await _insertMultipleComorbilidades([
          {
            'id_comorbilidades':
                comorbilidadActualizadaData['id_comorbilidades'],
            ...
            'usuario_modificacion':
                comorbilidadActualizadaData['usuario_modificacion'],
            'fecha_modificacion':
                comorbilidadActualizadaData['fecha_modificacion'],
  ...
  Future<void> eliminarComorbilidades(int idComorbilidad) async {
    try {
      String url =
          '$BASE_URL/v1/catalogo/captacion/delete-comorbilidades/$idComorbilidad';
  ...
  Future<void> _fetchAndUpdateComorbilidades() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        List<Map<String, dynamic>> comorbilidadesList =
            jsonResponse.map((comorbilidad) {
          return {
