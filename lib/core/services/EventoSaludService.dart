...
class EventoSaludService {
  ...
  Future<Database> _initDatabase() async {
    ...
    return await openDatabase(
      ...
      onCreate: (db, version) async {
        ...
        ''');
        await db.execute(
            'CREATE INDEX idx_eventos_salud_id ON eventos_salud(id_evento_salud)');
      },
    );
  }
  ...
  Future<List<Map<String, dynamic>>> _getAllEventosSaludLocal() async {
    try {
      ...
      final List<Map<String, dynamic>> maps =
          await db.query('eventos_salud', orderBy: 'nombre ASC');
      return maps;
    } catch (e) {
  ...
  Future<void> _insertMultipleEventosSalud(
      List<Map<String, dynamic>> eventosSaludList) async {
    try {
      final db = await database;
  ...
  Future<List<Map<String, dynamic>>> listarEventosSalud() async {
    ...
    List<Map<String, dynamic>> eventosSaludLocal =
        await _getAllEventosSaludLocal();
  ...
  Future<Map<String, dynamic>> agregarEventoSalud(
      Map<String, dynamic> nuevoEvento) async {
    try {
        ...
        await _insertMultipleEventosSalud([
          {
            ...
            'activo': nuevaEventoSaludData['activo']
                ? 1
  ...
  Future<Map<String, dynamic>> actualizarEventoSalud(
      int idEventoSalud, Map<String, dynamic> eventoActualizado) async {
    try {
      String url =
          '$BASE_URL/v1/catalogo/captacion/update-evento-salud/$idEventoSalud';
      final response = await httpService.put(url, eventoActualizado);

      if (response.statusCode == 200) {
        final decodedResponse = utf8.decode(response.bodyBytes);
        final eventoActualizadoData =
            jsonDecode(decodedResponse) as Map<String, dynamic>;
  ...
  Future<void> eliminarEventoSalud(int idEventoSalud) async {
    try {
      String url =
          '$BASE_URL/v1/catalogo/captacion/delete-evento-salud/$idEventoSalud';
  ...
  Future<void> _fetchAndUpdateEventosSalud() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        List<Map<String, dynamic>> eventosSaludList =
            jsonResponse.map((evento) {
          return {
