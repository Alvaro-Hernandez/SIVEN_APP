...
class CaptacionResultadoBusqueda extends StatefulWidget {
  ...
  @override
  _CaptacionResultadoBusquedaState createState() =>
      _CaptacionResultadoBusquedaState();
}

class _CaptacionResultadoBusquedaState
    extends State<CaptacionResultadoBusqueda> {
  ...
  Future<void> _saveResults(List<Map<String, dynamic>> resultados) async {
    ...
    String jsonResultados =
        jsonEncode(resultados); // Convertir la lista de mapas a JSON
    await prefs.setString('resultados_busqueda',
        jsonResultados); // Guardar el JSON en shared_preferences
    print(
        'Resultados de búsqueda guardados'); // Log para confirmar almacenamiento
  }
  ...
  Future<void> _loadSavedResults() async {
    ...
    if (jsonResultados != null) {
      List<Map<String, dynamic>> resultados =
          List<Map<String, dynamic>>.from(jsonDecode(jsonResultados));
      List<int> ids = resultados
      ...
      setState(() {
        ...
        resultadosFiltrados =
            resultadosBusqueda; // Inicialmente mostrar todos los resultados
        personaIds = ids;
      });

      // Imprimir los IDs en la terminal
      print(
          'IDs de las personas mostradas en CaptacionResultadoBusqueda: $personaIds');
    }
  }
  ...
  void _filtrarResultados(String query) {
    ...
    setState(() {
      if (query.isEmpty) {
        resultadosFiltrados = List.from(
            resultadosBusqueda); // Mostrar todos los resultados si no hay filtro
      } else {
        resultadosFiltrados = resultadosBusqueda.where((persona) {
          ...
          String expediente =
              (persona['codigo_expediente'] ?? '').toLowerCase();
  ...
  Widget build(BuildContext context) {
    ...
    if (argumentos != null) {
      List<Map<String, dynamic>> resultados =
          List<Map<String, dynamic>>.from(argumentos['resultados'] ?? []);
      List<int> personaIdsRecibidos =
          List<int>.from(argumentos['personaIds'] ?? []);

      // Imprimir los IDs en la terminal
      print(
          'IDs recibidos en CaptacionResultadoBusqueda: $personaIdsRecibidos');
      ...
      if (resultados != resultadosBusqueda) {
        setState(() {
          resultadosBusqueda = resultados;
          resultadosFiltrados = List.from(
              resultadosBusqueda); // Mostrar también los resultados filtrados
          ...
          child: IconButton(
            ...
            onPressed: () {
              Navigator.pushNamed(context,
                  '/captacion_busqueda_por_nombre'); // Volver a la pantalla inicial
            },
          ),
        ),
              ...
              child: Column(
                ...
                children: [
                  ...
                  FiltroPersonaWidget(
                    ...
                    onChanged:
                        _filtrarResultados, // Filtrar resultados dinámicamente
                  ),
                  const SizedBox(height: 20),
                  ...
                  Column(
                    children: resultadosFiltrados.map((persona) {
                      ...
                      String departamento =
                          persona['departamento'] ?? 'Sin departamento';
                        ...
                        children: [
                          CardPersonaWidget(
                            identificacion: persona['cedula'] ??
                                'Sin cédula', // Mostrar identificación
                            expediente: persona['codigo_expediente'] ??
                                'Sin expediente', // Mostrar expediente
                            ...
                            onBotonPressed: () async {
                              ...
                              Navigator.pushNamed(
                                ...
                                arguments:
                                    persona, // Pasar todos los datos del paciente
                              );
                            },
                          ),
                          const SizedBox(
                              height: 10), // Espaciado entre las tarjetas
                        ],
                      );
                    }).toList(),
                  ),
