...
class CuartaTarjetaState extends State<CuartaTarjeta> {
  ...
  Map<String, dynamic> getData() {
    return {
      ...
      'selectedEstablecimientoDiagnosticoId':
  ...
  Future<void> _cargarDiagnosticos() async {
    try {
      List<Map<String, dynamic>> diagnosticos =
          await widget.diagnosticoService.listarDiagnosticos();
  ...
  Future<void> _cargarResultadosDiagnostico() async {
    try {
      List<Map<String, dynamic>> resultados = await widget
          .resultadoDiagnosticoService
          .listarResultadosDiagnostico();
  ...
  List<String> validate() {
    ...
    if (densidadFalciparumEASController.text.isEmpty) {
      ...
      if (!RegExp(r'^\d+$').hasMatch(densidadFalciparumEASController.text)) {
        errors.add(
            'Densidad Parasitaria Falciparum EAS debe contener solo dígitos');
      }
    }
    ...
    if (densidadFalciparumESSController.text.isEmpty) {
      ...
      if (!RegExp(r'^\d+$').hasMatch(densidadFalciparumESSController.text)) {
        errors.add(
            'Densidad Parasitaria Falciparum ESS debe contener solo dígitos');
      }
    }
                ...
                : CustomTextFieldDropdown(
                    ...
                    onChanged: (selectedOption) {
                      ...
                      if (selectedOptionObj.id != -1) {
                        ...
                        print(
                            'Opción seleccionada para $label: ID=${selectedOptionObj.id}, Nombre=${selectedOptionObj.name}');
                      } else {
                        onChanged(null);
                        controller.text = '';
                      }
  ...
  Future<void> _abrirDialogoSeleccionRedServicioDiagnostico() async {
    ...
    if (result != null) {
      ...
      print(
          'ID seleccionado SILAIS Diagnóstico: $_selectedSILAISDiagnosticoId');
      print(
          'ID seleccionado Establecimiento Diagnóstico: $_selectedEstablecimientoDiagnosticoId');
    }
  }
            ...
            children: [
              ...
              _buildCustomDropdownField(
                ...
                onChanged: (selectedId) {
                  ...
                  print(
                      'Resultado Diagnóstico seleccionado ID: $_selectedResultadoDiagnosticoId');
                },
